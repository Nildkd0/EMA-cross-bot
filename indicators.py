...
def detect_ema_cross(df: pd.DataFrame, ema_col: str):
    """
    Look at the last *closed* candle vs the one before it and report whether
    price crossed the single configured EMA line.

    Returns "up", "down", or None (no cross on the last closed candle).
    """
    if len(df) < 3:
        return None
    ...
    prev_above = prev["close"] > prev[ema_col]
    last_above = last["close"] > last[ema_col]
    if prev_above != last_above:
        return "up" if last_above else "down"
    return None
