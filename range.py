...
def _send_with_chart(df, symbol: str, timeframe: str, range_high: float, range_low: float,
                       caption: str, direction: str = None):
    try:
...
def check_range(symbol: str, timeframe: str, df, state: dict) -> bool:
    ...
    if not entry["in_range"]:
        ...
        if width_pct <= config.RANGE_WIDTH_PCT:
            ...
            _send_with_chart(df, symbol, timeframe, candidate_high, candidate_low, caption)
            return True
        return False
    ...
    if close > entry["high"]:
        ...
        _send_with_chart(df, symbol, timeframe, entry["high"], entry["low"], caption, direction="up")
        ...
        _send_with_chart(df, symbol, timeframe, entry["high"], entry["low"], caption, direction="down")
        st.set_range_entry(state, symbol, timeframe, {"in_range": False, "high": None, "low": None})
        return True
