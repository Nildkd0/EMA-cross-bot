...
Supports Telegram's forum "topics" feature via an optional thread_id — if
given, the message/photo is posted into that specific topic thread instead
of the group's default General topic. Requires the target chat to be a
forum-enabled supergroup and the bot to have permission to post there.
"""
import logging
...
def send_message(text: str, parse_mode: str = "Markdown", thread_id: int = None):
    try:
...
def send_photo(photo_path: str, caption: str = "", parse_mode: str = "Markdown", thread_id: int = None):
    try:
        data = {"chat_id": config.TELEGRAM_CHAT_ID, "caption": caption, "parse_mode": parse_mode}
        if thread_id is not None:
            data["message_thread_id"] = thread_id
        with open(photo_path, "rb") as f:
            resp = requests.post(_url("sendPhoto"), data=data, files={"photo": f}, timeout=30)
        if not resp.ok:
