...
def _send_with_chart(symbol: str, session: str, range_high: float, range_low: float,
                       caption: str, chart_end_ts: int, thread_id: int, direction: str = None):
...
def _check_symbol(symbol: str, session: str, start_ist_hhmm: str, state: dict, now_dt: datetime):
    ...
    if not entry["formed"]:
        ...
        _send_with_chart(
            symbol, session, range_high, range_low, caption, end_ts,
            thread_id=config.TELEGRAM_THREAD_ORB_FORMING,
        )
        return
    ...
    if not entry["up_alerted"] and close > entry["high"]:
        ...
        _send_with_chart(
            symbol, session, entry["high"], entry["low"], caption, now_ts,
            thread_id=config.TELEGRAM_THREAD_ORB_BREAKOUT, direction="up",
        )
        ...
        _send_with_chart(
            symbol, session, entry["high"], entry["low"], caption, now_ts,
            thread_id=config.TELEGRAM_THREAD_ORB_BREAKOUT, direction="down",
        )
    else:
        st.set_orb_entry(state, symbol, session, date_str, entry)
