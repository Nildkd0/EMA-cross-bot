...
def scan_symbol(symbol: str, timeframe: str, resolution_seconds: int):
    ...
    daily_vwap = last_closed.get("vwap")
    if daily_vwap is not None and daily_vwap != daily_vwap:  # NaN check
        daily_vwap = None
    ...
    for cross in crosses:
        ...
        if daily_vwap is not None:
            position = "above" if last_closed["close"] > daily_vwap else "below"
            diff_pct = (last_closed["close"] - daily_vwap) / daily_vwap * 100
            vwap_tag = f"\nVWAP (daily): `{daily_vwap:.4f}` (price {position}, {diff_pct:+.2f}%)"
