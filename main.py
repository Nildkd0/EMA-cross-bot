...
def scan_symbol(symbol: str, timeframe: str, resolution_seconds: int):
    ...
    try:
        ...
        notifier.send_photo(chart_path, caption=caption, thread_id=config.TELEGRAM_THREAD_EMA_SUMMARY)
        os.remove(chart_path)
    except Exception as e:  # noqa: BLE001
        log.error("Chart/send failed for %s %s, falling back to text: %s", symbol, timeframe, e)
        notifier.send_message(caption, thread_id=config.TELEGRAM_THREAD_EMA_SUMMARY)
...
def summary_worker():
    ...
    while True:
        try:
            ...
            notifier.send_message(message, thread_id=config.TELEGRAM_THREAD_EMA_SUMMARY)
...
def main():
    ...
    notifier.send_message(
        ...
        thread_id=config.TELEGRAM_THREAD_EMA_SUMMARY,
    )
