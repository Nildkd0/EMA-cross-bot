...
(symbol + timeframe + ema + direction) until N candles have elapsed
since it last fired.
"""
import threading
...
class SignalState:
    ...
    @staticmethod
    def _key(symbol: str, timeframe: str, ema_label: str, direction: str) -> str:
        return f"{symbol}|{timeframe}|{ema_label}|{direction}"

    def should_fire(self, symbol: str, timeframe: str, ema_label: str, direction: str, candle_idx: int) -> bool:
        key = self._key(symbol, timeframe, ema_label, direction)
        with self._lock:
