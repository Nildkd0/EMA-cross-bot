...
def _get_optional_int(name: str, default: str):
    """Like _get_int, but the literal value 'none' (any case) returns None —
    used for settings like Telegram topic IDs where "don't use a topic,
    just post to the main chat" is a valid choice."""
    val = os.getenv(name)
    if val is None or val.strip() == "":
        val = default
...
class Config:
    ...
    # Telegram forum topic thread IDs — routes each alert type to its own
    # topic instead of one mixed feed. Leave any of these unset (empty
    # string) to fall back to the group's default General topic for that
    # category. Requires TELEGRAM_CHAT_ID to be a forum-enabled supergroup
    # (its real chat_id, e.g. -1003983425905 — NOT the shortened number
    # shown in a t.me/c/<that number>/<topic id> link).
    TELEGRAM_THREAD_EMA_SUMMARY = _get_optional_int("TELEGRAM_THREAD_EMA_SUMMARY", "8")
    TELEGRAM_THREAD_RANGE_FORMING = _get_optional_int("TELEGRAM_THREAD_RANGE_FORMING", "6")
    TELEGRAM_THREAD_RANGE_BREAKOUT = _get_optional_int("TELEGRAM_THREAD_RANGE_BREAKOUT", "7")
    TELEGRAM_THREAD_ORB_FORMING = _get_optional_int("TELEGRAM_THREAD_ORB_FORMING", "2")
    TELEGRAM_THREAD_ORB_BREAKOUT = _get_optional_int("TELEGRAM_THREAD_ORB_BREAKOUT", "5")
